import React from 'react';

/*///
 COMPONENT
*////
const Welcome = (props) => {

  return (
    <div>
      <div className="container-fluid black-with-blue">
        <div className="welcome-textbox">
          <ul>
            <li><span className="green-text">J R Leja Design NYC</span> showcases the web apps, games, and other</li>
            <li className="li-add-padding-bottom-20">software development work and art of <span className="green-text">Jasiu Leja</span>.</li>
            <li>Jasiu is a NYC-based Full Stack | Front End <span className="green-text">Software Developer</span></li>
            <li className="li-add-padding-bottom-20">with a passion for art and design.</li>
            <li>Thanks for visiting!</li>
          </ul>
        </div>
      </div>
    </div>
  );
};

export default Welcome;
